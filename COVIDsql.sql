...
SELECT location, date, total_cases, new_cases, total_deaths, population,  people_vaccinated,total_boosters, people_fully_vaccinated
FROM [CovidProject].[dbo].['owid-covid-data$']
ORDER BY location, date;
...
ALTER TABLE[CovidProject].[dbo].['owid-covid-data$']
...
DELETE FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE location = 'north korea';

---- (deleted North Korea data because it was not accurate at all)
...
SELECT location, population, date, total_cases, total_deaths, (total_deaths/total_cases)*100 AS fatality
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000'
ORDER BY fatality DESC;


--- shows the percentage of the population that got covid

SELECT location, population, date, total_cases, total_deaths, (total_cases/population)*100 AS case_fatality
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000'
...
SELECT location, date, total_cases, population, total_deaths, (total_deaths/population)*100 AS death_population
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000'
ORDER BY death_population DESC;
...
SELECT continent, (SUM(total_deaths)/SUM(total_cases))*100  AS fatality
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000'
GROUP BY continent
ORDER BY fatality DESC;
...
SELECT continent, (SUM(total_cases)/SUM(population))*100  AS fatality
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000'
GROUP BY continent
ORDER BY fatality DESC;
...
SELECT continent, (SUM(total_deaths)/SUM(population))*100  AS fatality
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000'
GROUP BY continent
ORDER BY fatality DESC;
...
SELECT SUM(total_cases) AS total_case, SUM(total_deaths) AS total_deaths, (SUM(total_deaths)/SUM(total_cases))*100  AS fatality
FROM [CovidProject].[dbo].['owid-covid-data$']
WHERE date = '2022-07-28 00:00:00.000';
...
SELECT continent, location, date,population, people_vaccinated,total_boosters, people_fully_vaccinated,
...
WITH popvac ( continent, location, date, population,people_vaccinated, total_boosters, people_fully_vaccinated, total_vacc)
AS
(
SELECT continent, location, date,population, people_vaccinated,total_boosters, people_fully_vaccinated,
(people_vaccinated + total_boosters + people_fully_vaccinated ) AS total_vacc 
...
SELECT *, (total_vacc/population) * 100 AS percentage
FROM popvac
...
SELECT continent, location, date, population, total_vaccinations, total_cases, total_deaths,
(total_vaccinations/population)* 100 AS percentage
FROM [CovidProject].[dbo].['owid-covid-data$']
ORDER BY percentage DESC;

---(percentage of the population vaccinated is more than 100%, this could be due to multiple booster and vaccination taken per person)
