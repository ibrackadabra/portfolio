...
SELECT 
  * 
FROM 
  [HotelProject].[dbo].['2018$'];

		--- 2019 data
SELECT 
  * 
FROM 
  [HotelProject].[dbo].['2019$'];

		--- 2020 data
SELECT 
  * 
FROM 
  [HotelProject].[dbo].['2020$'];
...
DROP 
  TABLE if exists hotels CREATE TABLE hotels (
    hotel varchar(50), 
    is_canceled numeric, 
    lead_time numeric, 
...
WITH fullhotel AS 
  ...
  FROM 
    hotels h 
    LEFT JOIN [HotelProject].[dbo].[market_segment$] ms ON h.market_segment = ms.market_segment 
    LEFT JOIN [HotelProject].[dbo].[meal_cost$] mc on h.meal = mc.meal
	)

        --- creating revenue column
SELECT 
  *, ((stays_in_week_nights + stays_in_weekend_nights)* adr)* Discount AS revenue 
from 
  fullhotel;
...
SELECT 
  hotel, 
  arrival_date_year, 
  SUM(((stays_in_week_nights + stays_in_weekend_nights)* adr)- discount) AS revenue 
FROM 
  hotels h 
  LEFT JOIN [HotelProject].[dbo].[market_segment$] ms ON h.market_segment = ms.market_segment 
  LEFT JOIN [HotelProject].[dbo].[meal_cost$] mc on h.meal = mc.meal 
